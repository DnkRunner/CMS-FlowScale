...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (!$errors) {
    ...
    // Dodaj post
    $stmt = $pdo->prepare("INSERT INTO `{$prefix}posts` (title,slug,content,status,template,author_id,published_at,scheduled_at,featured_image,show_featured_image,show_title,excerpt,meta_title,meta_description) VALUES (:t,:s,:c,:st,:tp,:a,:p,:sch,:fi,:sfi,:stitle,:ex,:mt,:md)");
    $stmt->execute([
      ':t'=>$titleV, ':s'=>$slugV, ':c'=>$contentV, ':st'=>$statusV, ':tp'=>$templateV, 
      ':a'=>$_SESSION['user_id'], ':p'=>$published_at, ':sch'=>$scheduled_at, ':fi'=>$featuredImageV, ':sfi'=>$showFeaturedV, ':stitle'=>$showTitleV,
      ':ex'=>$excerptV, ':mt'=>$metaTitleV, ':md'=>$metaDescriptionV
    ]);
    ...
    if (!empty($categoriesV)) {
      ...
      foreach ($categoriesV as $catId) {
        $stmt->execute([':pid'=>$postId, ':cid'=>$catId]);
      }
    }
...
<form method="post">
  ...
  <textarea class="input" name="excerpt" rows="3" placeholder="Krótki opis wpisu..."></textarea>
  
  <label>Meta tytuł (SEO)</label>
  <input class="input" name="meta_title" placeholder="Tytuł dla wyszukiwarek">
  
  <label>Meta opis (SEO)</label>
  <textarea class="input" name="meta_description" rows="2" placeholder="Opis dla wyszukiwarek..."></textarea>
  <label>Treść</label>
  ...
  <div style="display:flex;gap:10px;margin-top:8px">
    ...
    <div><label>Szablon</label>
      <select name="template" class="input">
        ...
        <option value="blank">Pusty</option>
      </select>
    </div>
  </div>
  
  <div id="scheduledField" style="margin-top: 10px; display: none;">
